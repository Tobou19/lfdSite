...
export default function ManageBeneficiaries() {
  ...
  const initialAppointments = {
    1: [
      { id: 1, date: "2025-10-01", time: "10:00", notes: "Première séance" },
      {
        ...
        date: "2025-10-15",
        time: "14:00",
        notes: "Suivi intermédiaire",
      },
    ],
    2: [
      { id: 3, date: "2025-10-05", time: "09:00", notes: "Séance de groupe" },
    ],
    3: [],
  };

  const initialDocs = {
    1: [{ id: 1, name: "Bilan_initial.pdf", uploadedAt: "2025-09-01" }],
    2: [],
    3: [],
  };
  ...
  const filteredList = useMemo(() => {
    ...
    return beneficiaries.filter((b) => {
      ...
      const matchesTherapy =
        !therapyFilter ||
        (therapies[b.id] &&
          therapies[b.id].some((t) => t.name === therapyFilter));
  ...
  async function handleDeleteBeneficiary(id) {
    if (
      !window.confirm(
        "Supprimer ce bénéficiaire ? Toutes ses données seront supprimées."
      )
    )
      return;
    ...
    try {
      ...
      alert(
        "Impossible de supprimer ce bénéficiaire. Vérifiez que l'ID existe."
      );
    }
  }
  ...
  function addTherapy(name) {
    ...
    setTherapies((prev) => ({
      ...prev,
      [selected.id]: [...(prev[selected.id] || []), { id, name }],
    }));
  }
  ...
  function removeTherapy(tid) {
    ...
    setTherapies((prev) => ({
      ...prev,
      [selected.id]: (prev[selected.id] || []).filter((t) => t.id !== tid),
    }));
  }
  ...
  function removeAppointment(id) {
    ...
    setAppointments((prev) => ({
      ...prev,
      [selected.id]: (prev[selected.id] || []).filter((a) => a.id !== id),
    }));
  }
  ...
  function removeDoc(id) {
    ...
    setDocs((prev) => ({
      ...prev,
      [selected.id]: (prev[selected.id] || []).filter((d) => d.id !== id),
    }));
  }
...
`;
  ...
  function goToCarnet() {
    if (!selected) return;
    window.location.href = `/dashboard/carnet/${selected.id}`;
  }
        ...
        <div className="flex items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Ajoute, édite, filtre et gère documents, thérapies et rendez-vous.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-green-600 text-white px-4 py-2 rounded-md shadow hover:bg-green-500 text-sm"
            >
                ...
                filteredList.map((b) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={goToCarnet}
                        className="text-sm px-3 py-2 bg-purple-50 text-purple-700 rounded"
                      >
                        Aller au carnet
                      </button>
            ...
            <div className="bg-white rounded-xl shadow p-4 h-[70vh] overflow-y-auto">
              {selected ? (
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-semibold text-green-600">
                        {(appointments[selected.id] || []).length}
                      </div>
                      <div className="text-xs text-gray-400">
                        Thérapies: {(therapies[selected.id] || []).length}
                      </div>
                    </div>
                  </div>
                  <hr className="my-4" />
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="Ajouter une thérapie"
                        className="flex-1 px-3 py-2 border rounded-md text-sm"
                      />
                      <button
                        onClick={() => {
                          if (therapyInput.trim()) {
                            addTherapy(therapyInput.trim());
                            setTherapyInput("");
                          }
                        }}
                  ...
                  <div>
                      ...
                      {(docs[selected.id] || []).map((d) => (
                        ...
                        >
                          <div>
                            ...
                            <div className="text-xs text-gray-400">
                              Téléversé: {d.uploadedAt}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => alert(`Aperçu simulé: ${d.name}`)}
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="Nom du document (ex: bilan.pdf)"
                        className="flex-1 px-3 py-2 border rounded-md text-sm"
                      />
                      <button
                        onClick={() => {
                ...
                <EmptyState
                  ...
                  subtitle="Cliquer sur un nom à gauche pour voir / gérer les détails."
                />
              )}
            </div>
              ...
              {selected ? (
                <>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-sm text-gray-500">
                      {(appointments[selected.id] || []).length} total
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 mb-4">
                    {(appointments[selected.id] || []).length === 0 ? (
                      ...
                      (appointments[selected.id] || []).map((a) => (
                        <div
                          key={a.id}
                          className="p-3 rounded border bg-gray-50 flex justify-between items-start"
                        >
                          <div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        onClick={() => handleDeleteBeneficiary(selected.id)}
                        className="text-sm px-3 py-2 bg-red-50 text-red-600 rounded"
                      >
                        Supprimer profil
                      </button>
                      <button
                        onClick={downloadFollowUpSheet}
                        className="text-sm px-3 py-2 bg-emerald-50 text-emerald-700 rounded"
                      >
                        Télécharger fiche de suivi
                      </button>
                      <button
                        onClick={() => setShowQuickAppt(!showQuickAppt)}
                        className="text-sm px-3 py-2 bg-blue-50 text-blue-700 rounded"
                      >
                        {showQuickAppt
                          ? "Annuler rendez-vous rapide"
                          : "Programmer rendez-vous rapide"}
                      </button>
                ...
                <EmptyState
                  title="Rendez-vous & Actions"
                  subtitle="Sélectionne un bénéficiaire pour gérer ses rendez-vous et actions rapides."
                />
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold">
                  {editing ? "Éditer bénéficiaire" : "Ajouter un bénéficiaire"}
                </h3>
                <button
                  onClick={() => {
                    setShowAddModal(false);
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <input
                  ...
                  value={form.idCarnet}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, idCarnet: e.target.value }))
                  }
                />
              </div>
